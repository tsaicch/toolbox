...
GITLAB_CHART_VERSION="$new_major.$minor.$patch"
echo $GITLAB_CHART_VERSION
env_init()
{
...
helm template gitlab-temp gitlab/gitlab --version $GITLAB_CHART_VERSION --set global.hosts.domain=DOMAIN   --set certmanager-issuer.email=me@example.com|grep image:|awk -F: '{print $2":"$3}'|sed 's/"//g'|grep -v IfNotPrese|sed 's/@sha256//g' |sed 's/^[ \t]*//g'|sort|uniq|tee $GITLAB_VERSION-gitlab-image-list
